...
public:
  ...
  inline static int atom_hash(AtomT x, uint32_t mask) { return (x & mask); }
  ...
  void resize(uint32_t new_size) {
    self_pointer *ndata =
        (new_size == 0) ? nullptr : (new self_pointer[new_size]);
...
template <typename AtomT, typename NodeT> struct TrieIteratorInternal {
  ...
  typename NodeT::value_type &get_value() const {
    NodeT *top = const_cast<NodeT *>(
        m_ptrs.empty() ? m_root : NodeT::value(m_ptrs.back()));
  ...
  bool step_fore() {
    ...
    if (up != nullptr) {
      do {
        ++m_ptrs.back();
      } while (m_ptrs.back() != up->end() and
               NodeT::value(m_ptrs.back()) == nullptr);
  ...
  void push(traverse_ptr it) { m_ptrs.push_back(it); }
};

typedef uint32_t trie_offset_t;

template <typename ValueT> struct ValueHolder {
private:
...
template <> struct ValueHolder<SetCounter> {
private:
  int count = 0;
...
template <typename AtomT, typename ValueT, size_t CMinChunkSize = 0,
          typename NodeImpl = typename detail::TrieNodeSelector<
              AtomT, ValueT, CMinChunkSize>::type>
struct trie_map {
private:
...
private:
  ...
  template <typename KeyIterator>
  NodeT *insert_edge(NodeT *parent, KeyIterator it, KeyIterator end,
  ...
  void insert_value(NodeT &at, const value_type &value,
...
public:
  ...
  private:
    ...
    explicit iterator(IteratorInternalT *a_impl) : _impl(a_impl) {
      normalize();
    }
  ...
  public:
     ...
     */
    iterator clone() {
      return (_impl.get() == nullptr) ? iterator()
...
private:
   ...
   */
  template <typename KeyIterator, typename A, typename B, typename C,
            typename D, typename E>
  inline void general_search(NodeT *n, KeyIterator it, KeyIterator end,
                             A exactMatchAction, B noNextEdgeAction,
                             C endInTheMiddleAction, D splitInTheMiddleAction,
                             E edgeAction) {
...
private:
  template <typename _ValueT, typename = typename std::enable_if<std::is_same<
                                  _ValueT, SetCounter>::value>::type>
  struct SetSpecific {};

public:
  template <typename KeyIterator, typename _ValueT = ValueT,
            typename = SetSpecific<_ValueT>>
  ...
  bool contains(KeyIterator it, KeyIterator end) {
    ...
    general_search(
        ...
        [](NodeT *, KeyIterator) {}, [](NodeT *, key_iterator) {},
        [](NodeT *, key_iterator, KeyIterator) {}, [](NodeItr, KeyIterator) {});
...
private:
  ...
  iterator find_prefix_int(NodeT *root_node, KeyIterator it, KeyIterator kend,
                           CallbackType exactMatch) {
    iterator output;
...
public:
  template <typename KeyIterator, typename CallbackType>
  iterator find_prefix(KeyIterator it, KeyIterator kend,
                       CallbackType exactMatch) {
    if (edges.empty()) {
      return end();
    }
  ...
  /* NOTE : this specialization is needed to catch bool as reference, not as
   * value */
  template <typename KeyIterator>
  ...
  template <typename KeyIterator, typename CallbackType>
  iterator find_prefix(const iterator &base, KeyIterator it, KeyIterator kend,
                       CallbackType exactMatch) {
 ...
 */
template <typename AtomT> struct CStrIterator : std::forward_iterator_tag {
private:
  AtomT *m_str;
